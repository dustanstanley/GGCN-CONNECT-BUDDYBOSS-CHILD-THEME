...
<div class="<?php echo ($rtl) ? 'left-col' : 'right-col'; ?><?php if($woocommerce) { echo ' woocommerce'; } ?>" <?php if ( !is_user_logged_in() ) { echo "title='Connect is a secure network sponsored by the Global Great Commission network to see the mission world better CONNECT, COMMUNICATE AND COLLABORATE.'"; } ?>>

<?php if ( !is_user_logged_in() ) { echo "<img src='https://ggcn.org/wp-content/uploads/2017/06/Connect-Logo-Small-Transparent.png' style='max-height: 32px;'>"; } ?>
			...
			<?php if ( !is_user_logged_in() && buddyboss_is_bp_active() && bp_hide_loggedout_adminbar( false ) ) : ?>
                ...
                <?php } ?>
                    <?php if ( buddyboss_is_bp_active() && bp_get_signup_allowed() ) : ?>
                        ...
                        <a href="/register" class="register screen-reader-shortcut"><?php _e( 'Register', 'boss' ); ?></a>
                    <?php endif; ?>
